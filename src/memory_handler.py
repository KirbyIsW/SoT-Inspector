...
class SoTMemoryHandler:
    ...
    def __init__(self):
        ...
        g_objects_offset = self.rm.read_ulong(base_address + self.rm.g_object_base + 3)
        g_objects = base_address + self.rm.g_object_base + g_objects_offset + 7
        logger.info(f"SoT gObject Address: {hex(g_objects)}")
        ...
        self.Dumper.load_dump()

        logger.info(f"Loaded dump in {round(time.time() - start_time, 4)} seconds")

        globals.local_player = self.load_local_player()
        globals.local_player_controller = self.rm.read_ptr(
            globals.local_player + self.Dumper.find_sdk_offset('UPlayer.PlayerController')
        )
        globals.local_root_component = globals.local_player_controller + self.Dumper.find_sdk_offset('AActor.RootComponent')
        globals.idle_dc_address = globals.local_player_controller + self.Dumper.find_sdk_offset('AOnlineAthenaPlayerController.IdleDisconnectEnabled')
        ...
        glb_elapsed_time = time.time() - glb_start_time
        Helper.logger.info(f"SoTMemoryReader object created with an elapsed time of {round(glb_elapsed_time, 4)} seconds")
    ...
    def update_my_coords(self):
        ...
        manager = self.rm.read_ptr(
            globals.local_player_controller + self.Dumper.find_sdk_offset('APlayerController.PlayerCameraManager')
        )
        self.my_coords = self._coord_builder(
            manager,
            self.Dumper.find_sdk_offset('APlayerCameraManager.CameraCache')
            + self.Dumper.find_sdk_offset('FCameraCacheEntry.POV'),
            fov=True)
    ...
    def load_local_player(self) -> int:
        ...
        game_instance = self.rm.read_ptr(
            self.u_world + self.Dumper.find_sdk_offset('UWorld.OwningGameInstance')
        )
        local_player = self.rm.read_ptr(
            game_instance + self.Dumper.find_sdk_offset('UGameInstance.LocalPlayers')
        )
    ...
    def read_all_actors(self):
        ...
        self.levels_ptr = self.rm.read_ptr(self.u_world + self.Dumper.find_sdk_offset("UWorld.Levels"))
        self.u_level_count = self.rm.read_uint32(self.u_world + self.Dumper.find_sdk_offset("UWorld.Levels") + 8)
        ...
        for x in range(0, actor_data[1]):
            ...
            if actor_id not in self.actor_name_map and actor_id != 0:
                try:
                    ...
                    logger.error(f"Unable to find actor name: {e} (Id: {actor_id})")
