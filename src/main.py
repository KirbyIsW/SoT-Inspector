...
simples = ["char", "bool", "float", "int8_t", "int16_t", "int32_t", "int64_t", "uint8_t", "uint16_t", "uint32_t", "uint64_t", "FText", "FName", "FString", "bit bool", "double"]
...
chosen_listbox_item_change_type = ""
...
def open_search_all_actors_for_address(search_type: str, item_id: str = None):
    ...
    maxDepth = simpledialog.askinteger("Max Search Depth", "Please enter max search depth (Higher values will take longer)", initialvalue=10, minvalue=1)
    ...
    def search_prop_address(mainProp: Union[SDKProperty, SDKClass], address: int, isClass: bool = False, search_address: int = 0, base_address: int = 0):
        propClass = mainProp
        if not isClass:
            propClass = mem.Dumper.get_class_from_name(mainProp.type_name)

            if not propClass:
                return
            
        if mainProp.temp_chain_depth > maxDepth:
            return
        
        props_to_scan = {}

        for prop in propClass.properties:
            ...
            if prop.is_array:
                if prop.is_pointer:
                    ...
                    typesize = mem.Dumper.get_class_size_from_name(prop.type_name)
                for i in range(prop.tArray_length):
                    array_index_address = prop.tArray_data + i * typesize

                    if array_index_address == search_address:
                        found_vals[prop.temp_parent_chain] = base_address

                    if prop.is_pointer:
                        ptr_address = mem.rm.read_ptr(array_index_address)
                        props_to_scan[ptr_address] = prop
                    else:
                        props_to_scan[array_index_address] = prop
                ...
                continue
            elif prop.is_enum or prop.is_unknown_data or prop.is_bit_size or prop.is_map:
                continue
            else:
        ...
        if propClass.super_class_name:
            parentClass = mem.Dumper.get_class_from_name(propClass.super_class_name)
            if parentClass:
                ...
                if search_type == "AllActorsOnePath":
                    if len(found_vals) > 1:
                        ...
                        for chain, address in found_vals.items():
                            split = chain.split(".")
                            if len(split) > len(longest_chain.split(".")) or longest_chain == "":
                                longest_chain = chain
    ...
    if len(found_vals) == 0:
        ...
        found_vals = dict(sorted(found_vals.items(), key=lambda item: len(item[0].split('.'))))
...
def add_to_render_pos_list(address, name, item):
    for tracked_vector in tracked_vectors:
        if tracked_vector.address == address:
            messagebox.showerror("Can't add vector", "Vector is already tracked! Check tracked vectors in the \"Tracked Vectors\" tab")
            return
...
def type_to_byte_count(type: str) -> int:
    ...
    if type in tbit64:
        ...
        return mem.Dumper.get_class_size_from_name(type)
...
def get_chosen_listbox_item(listbox: Listbox, prop: 'UnknownProperty'):
    ...
    if selected_indices:
        ...
        type = chosen_item.split(":")[0]
        if isinstance(prop, str):
            change_item_type(prop, type)
        else:
...
def open_type_changing_window(filtered_result: dict, prop: Union['UnknownProperty', 'SDKProperty', str]):
    ...
    if isinstance(prop, str):
        text = inspector_tree.item(prop)["text"]
        values = inspector_tree.item(prop)["values"]
        prop_name = text.split(" : ")[0]
    ...
    class_instance_window.title(f"Change Type | {prop_name}")
...
def toggle_bit_bool(address, bit) -> bool:
    bit = simpledialog.askinteger("Bit Offset", "Enter bit: ", initialvalue=0)
...
class UnknownProperty:
    ...
    def update(self, type: str, offset: int = None, expandable: bool = None, buffer: int = None) -> None:
        ...
        if self.type == "Class Instance":
            self.size = mem.Dumper.get_class_size_from_name(self.subtype)
            if self.size <= 0:
                self.size = 8
        elif self.type == "Void Pointer":
            self.size = 8
...
def change_type(item: str, type: str) -> None:
    ...
    size = mem.Dumper.get_class_size_from_name(type)
    ...
    if type == "Class Instance":
        ...
        class_items_that_fit = mem.Dumper.get_classes_that_fit(remaining_bytes)
        ...
        if state:
            ...
            if property:
                ...
                if data["IsSimpleType"] and "UnknownData" not in name and not data["IsArray"]:
                    ...
                    inspector_tree.item(item, text=prop_text)
                elif "UnknownData" in name and not data["IsBitSize"]:
                    if inspector_tree.item(item, "open"):
                        reload_unknown_data_node(item)
...
def change_sub_type(prop: UnknownProperty):
    if prop.type == "Class Instance":
        ...
        class_items_that_fit = mem.Dumper.get_classes_that_fit(remaining_size)
    else:
        ...
        for prop in propClass.properties:
            ...
            if prop.is_array:
                ...
                for i in range(prop.tArray_length):
                    ...
                    if array_index_address == address:
                        found_vals[prop.temp_parent_chain] = f'->{hex(search_val)}'

                    if prop.is_pointer:
                        ptr_address = mem.rm.read_ptr(array_index_address)
                        if ptr_address == address:
                            found_vals[prop.temp_parent_chain] = f'->{hex(search_val)}'
                ...
                continue
            elif prop.is_enum or prop.is_unknown_data or prop.is_bit_size:
                continue
            else:
                struct_address = address + prop.offset
                props_to_scan[struct_address] = prop
...
def change_item_type(item_id: str, param: Union[int, str]):
    tree_node = inspector_tree.item(item_id)

    if isinstance(param, int):
        class_items_that_fit = Helper.class_size_map
        sorted_dict = dict(sorted(class_items_that_fit.items(), key=lambda item: item[1], reverse=True))
        open_type_changing_window(sorted_dict, item_id)
        return
    
    opened = inspector_tree.item(item_id, "open")
    ...
    old_type = get_name_from_id(item_id)

    try:
        data = eval(new_values[0])
        old_type = data["TypeName"]
        data["TypeName"] = param
        new_values[0] = data
        new_label = f'{param}*' + tree_node["text"].split("*")[1]
    except:
        new_label = f'{param} : ' + tree_node["text"].split(" : ")[1]
        new_values[0] = param

    new_values.append(old_type)
    
    inspector_tree.item(item_id, text=new_label, values=new_values)
    if opened:
        inspector_tree.item(item_id, open=True)
        inspector_tree.see(item_id)
        inspector_tree.focus(item_id)
        on_insp_node_open()
...
def show_insp_context_menu(event: tk.Event):
    ...
    if len(values) == 1 and isinstance(values[0], int):
        ...
        if values:
            ...
            try:
                data = eval(values[0])
                if data["IsPointer"] and not data["IsArray"]:
                    insp_context_menu.add_command(label="Copy Pointed Address", command=lambda: copy_value(hex(address)))
            except: pass
            address = values[1]
            try:
                try:
        ...
        if " : 0x" in rawlabel and "->" not in rawlabel:
            ...
            if len(values) == 4:
                ...
                insp_context_menu.add_command(label="Change Class Type", command=lambda: change_item_type(item, address))
        ...
        try:
            dontAddResetSep = False
            if not addedCopySep:
                insp_context_menu.add_separator()
                addedCopySep = True
                dontAddResetSep = True
            if "UnknownData" in data["Name"] and data["TypeName"] != "bit bool":
                if not dontAddResetSep:
                    insp_context_menu.add_separator()
            ...
            if data["TypeName"] == "FVector":
                if json_config["rendering"]:
                    chain_str = get_parent_name_chain(item)
                    chain_str = f'{chain_str.split(".")[0]}.{chain_str.split(".")[-1]}'
                    insp_context_menu.add_command(label="Track Vector", command=lambda: add_to_render_pos_list(address, chain_str, item))
                else:
            ...
            if data["IsPointer"] and not data["IsArray"]:
                if len(values) == 4:
                    ...
                    insp_context_menu.add_command(label="Change Type", command=lambda: change_item_type(item, address))
            ...
            if converted_data["IsArray"]:
                ...
                for i in range(array_length):
                    if is_array_of_ptr:
                        ...
                        typesize = mem.Dumper.get_class_size_from_name(type_name)
...
def inspector_tree_on_class(tree: ttk.Treeview, class_name: str, item_id: str, address: int):
    ...
    if classobj.super_class_name and classobj.super_class_name != "None":
        parent_insert = tree.insert(item_id, "end", text=f"Super: {classobj.super_class_name}", tags=("monospaced",), values=(classobj.super_class_name, address,))
    ...
    for property in classobj.properties:
        ...
        if property.is_pointer:
            ...
            prop_values = (property.to_data(), address + property.offset)
            key_size = mem.Dumper.get_class_size_from_name(property.map_key_type)
            value_size = mem.Dumper.get_class_size_from_name(property.map_value_type)
            ...
            if property.tArray_length >= 1000:
                prop_text += " (Invalid)"
            
            prop_values = (property.to_data(), address + property.offset)
            prop_insert = tree.insert(item_id, "end", text=prop_text, tags=("monospaced",), values=prop_values)
            if property.tArray_length > 0 and property.tArray_length < 1000:
                tree.insert(prop_insert, "end", text="", tags=("dummy",))
            continue
    ...
    for address, actor in mem.address_name_map.items():
        ...
        if address in attached_actors.keys():
            if data is not None and actor not in actor_filters and data != "AActor":
                actor_tree.insert("", "end", text=f"{hex(address)} : {actor} (Valid)", tags=("monospaced", "attached"), values=data)
            else:
                actor_tree.insert("", "end", text=f"{hex(address)} : {actor} (Invalid)", tags=("monospaced", "attached"), values=data)
        else:
            if data is not None and actor not in actor_filters and data != "AActor":
...
def main():
    ...
    globals.json_config = json_config
